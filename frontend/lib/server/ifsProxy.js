// Server-only: proxies a GET against an IFS Cloud projection. Runs on the
// server so the OAuth2 client secret never reaches the browser bundle and so
// the call isn't blocked by the browser's CORS policy.

import { requestIfsToken, IfsAuthError } from './ifsAuth'
import { buildLiveDataUrl } from '../migrationStore'

export async function proxyIfsGet(request, dataset) {
  ...
  let dataUrl
  try {
    dataUrl = buildLiveDataUrl(body?.baseUrl, dataset)
  } catch {
  ...
  try {
    ...
    if (!dataRes.ok) {
      return Response.json(
        {
          success: false,
          error: `${dataset.title} request failed (${dataRes.status}). ${dataBody?.message || ''}`.trim(),
    ...
    return Response.json({ success: false, error: `Could not reach ${dataset.title}: ${err.message}` }, { status: 502 })
  }
}
